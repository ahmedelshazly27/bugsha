...
const {SITE_ASSET:ASSET, SiteLink:A, Arrow, BrandMark, CookieReveal, DownloadSection, MotionProvider, ScreenPhone, SiteFooter, SiteHeader} = window;
...
function useBagScrollScene(){
  useEffect(()=>{
    ...
    const items = Array.from(section.querySelectorAll("[data-bag-item]")).map(node=>({
      node,
      start:Number(node.dataset.start||0), end:Number(node.dataset.end||1),
      x:Number(node.dataset.x||0), lift:Number(node.dataset.lift||0), finalY:Number(node.dataset.finalY||0),
      rotateFrom:Number(node.dataset.rotateFrom||0), rotateTo:Number(node.dataset.rotateTo||0),
      scaleFrom:Number(node.dataset.scaleFrom||1), scaleTo:Number(node.dataset.scaleTo||1)}));
    const reduced = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
    let frameId=0, motionScale=1, layoutDirty=true, scrollDirty=true;
    let targetProgress=0, renderedProgress=0, previousTime=performance.now();

    const measure = () => { motionScale = scene.offsetWidth/650; layoutDirty=false; };
    const getProgress = () => {
    ...
    const tick = time => {
      ...
      if(scrollDirty){ targetProgress = reduced ? 1 : getProgress(); scrollDirty=false; }
    ...
    const schedule = () => { scrollDirty=true; if(!frameId){ previousTime=performance.now(); frameId=requestAnimationFrame(tick); } };
    const handleResize = () => { layoutDirty=true; schedule(); };
    measure();
    targetProgress = reduced ? 1 : getProgress();
    renderedProgress = targetProgress;
    render(renderedProgress);
    const ro = new ResizeObserver(handleResize); ro.observe(scene);
    window.addEventListener("scroll", schedule, {passive:true});
    window.addEventListener("resize", handleResize, {passive:true});
    return ()=>{ cancelAnimationFrame(frameId); ro.disconnect();
      window.removeEventListener("scroll", schedule); window.removeEventListener("resize", handleResize); };
  },[]);
}
...
function useSharedTableScene(){
  useEffect(()=>{
    ...
    const reduced = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
    let frameId=0, startScale=.72, startY=54;
    const measure = () => {
    ...
    return ()=>{ cancelAnimationFrame(frameId); window.removeEventListener("scroll", schedule); window.removeEventListener("resize", resize); };
  },[]);
}
...
function useAppStoryStep(){
  ...
  useEffect(()=>{
    ...
    const update = () => {
      ...
      if(nextStep !== activeStepRef.current){ activeStepRef.current = nextStep; setActiveAppStep(nextStep); }
    };
      ...
      <main>
        <section className="home-hero" id="top">
          <div className="home-hero-copy" data-reveal>
            ...
            <div className="hero-actions"><A className="button button-primary" href="/#waitlist">Join the waitlist <Arrow /></A><A className="text-link" href="/partners">List your kitchen <Arrow /></A></div>
          ...
          <div className="hero-scene" data-reveal="scale">
            <figure data-scroll-media="24"><img src={ASSET("bugsha-community-story.webp")} alt="A woman using her phone outside a neighborhood café" /></figure>
        ...
        <section className="mission-statement">
          ...
          <h2 data-reveal>We are building a nightly marketplace for the good food our cities’ kitchens made—but did not sell.</h2>
        </section>
          ...
          <div className="bag-pin">
            <div className="bag-layout">
              ...
              <div className="bag-visual" role="img" aria-label="A purple Bugsha bag unpacking a bouquet of fresh food as the page scrolls">
                <div className="bag-sun" aria-hidden="true"></div>
                <div className="bag-scene" aria-hidden="true">
                  <div className="bag-back">
                    <img src={ASSET("bugsha-animation-bag.webp")} alt="" />
                    <span className="bag-interior-shade"></span></div>
                  <div className="bag-food-clip">
                    <div className="bag-food-stage">
                      {bagBouquet.map(item=>(
                        <img key={item.name} className={`bag-item item-${item.name}`} src={ASSET(`bag-item-${item.name}.webp`)} alt=""
                          data-bag-item data-start={item.start} data-end={item.end} data-x={item.x} data-lift={item.lift}
                          data-final-y={item.finalY} data-rotate-from={item.rotateFrom} data-rotate-to={item.rotateTo}
                          data-scale-from={item.scaleFrom} data-scale-to={item.scaleTo} decoding="async" />))}
                    </div>
                  </div>
                  <div className="bag-front">
                    <img src={ASSET("bugsha-animation-bag.webp")} alt="" />
                    <span className="bag-opening-contact"></span></div>
                </div>
              </div>
            </div>
          ...
          <div className="app-story-pin">
            ...
            <div className="app-story-layout">
              <div className="app-story-device">
                <ScreenPhone active={activeAppStep} screens={appSteps.map(s=>({src:s.src, alt:`${s.kicker} in the Bugsha app`}))} /></div>
              <div className="app-story-copy">
                {appSteps.map((step,index)=>(
                  <article className={activeAppStep===index?"active":""} key={step.n}>
                    <span>{step.n} / 03</span><small>{step.kicker}</small><h3>{step.title}</h3><p>{step.body}</p></article>))}
              </div>
            </div>
        ...
        <section className="shared-story" data-shared-story>
          <div className="shared-story-pin">
            <figure className="shared-story-media" data-shared-media>
              <img src={ASSET("bugsha-shared-meal.webp")} alt="Friends sharing rescued food together around a table" loading="lazy" decoding="async" />
              <span className="shared-story-shade" data-shared-shade aria-hidden="true"></span></figure>
            ...
            <div className="shared-story-copy" data-shared-copy>
              ...
              <A className="button button-light" href="/how-it-works">See how Bugsha will work <Arrow /></A>
            </div>
          </div>
        </section>

        <section className="local-story">
          <figure data-reveal="scale" data-scroll-media="42">
            <img src={ASSET("bugsha-bakery-story.webp")} alt="A local baker arranging fresh bread and pastries" loading="lazy" decoding="async" />
            <figcaption>Made today · listed tonight</figcaption></figure>
          <div className="local-story-copy" data-reveal>
            ...
            <A className="text-link" href="/impact">Read what we are building <Arrow /></A>
          </div>
        </section>
          ...
          <div className="faq-list">
            {faqs.map(([question,answer],index)=>(
              <div className={`faq-item ${openFaq===index?"open":""}`} key={question}>
                <button onClick={()=>setOpenFaq(openFaq===index?-1:index)} aria-expanded={openFaq===index} aria-controls={`faq-answer-${index}`}>
                  <span>{question}</span><i /></button>
                <div id={`faq-answer-${index}`}><p>{answer}</p></div></div>))}
          </div>
        </section>
...
Object.assign(window,{ SiteHome:Home });
})();
