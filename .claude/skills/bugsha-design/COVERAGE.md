...
| `partner-request` edge function + `public.partner_request` table + two emails | `supabase/functions/partner-request/`, `supabase/functions/_shared/validate-partner.ts`, `supabase/migrations/20260914090000_partner_request.sql`, `supabase/functions/_shared/templates/partner-request-*.js`, previews in `emails/preview/` | Written; validator unit-tested under Node; **not deployed** — the waitlist Supabase project (`qrmyhruvnqmjwxcnkocj`) is currently **paused (INACTIVE)**, which also means the live waitlist form on bugsha.app cannot be saving signups right now |
