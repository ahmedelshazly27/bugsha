...
A two-sided marketplace for Kuwait: restaurants, bakeries, cafés and co-ops sell end-of-day surplus as discounted **surprise bags**, reserved in-app and collected in person inside a fixed pickup window. Three surfaces share this system — a **consumer app** (mobile-first, iOS/Android), a **partner app** (mobile + tablet web) and the internal **ops console** (desktop web). All three are Expo apps in `bugsha-platform`; the kits here recreate every screen of each at every stage — see `COVERAGE.md` for the screen-by-screen map against the platform database and what was verified.
...
- `readme.md` — this file.
- `COVERAGE.md` — every kit screen mapped to the platform table / RPC / enum it mirrors, with what was verified against the live database and what is assumed.
...
**Money.** `PriceTag`, `MapPin` and `PayoutCard` take a `currency` and infer the minor-unit digits from it — three for KD, two for everything else (EGP) — or an explicit `decimals`.
...
**App screens** (`assets/screens/`) — consumer: `browse-en`, `detail-en`, `code-en`, `browse-ar`, `detail-ar`, `code-ar`, plus the stages `consumer-language-en`, `consumer-code-en`, `consumer-payment-eg-en`, `consumer-reserved-qr-en`, `consumer-wallet-ar`, `consumer-report-en`; partner: `partner-tonight-en/-ar`, `partner-list-en/-ar`, `partner-ledger-en`, `partner-analytics-en`, plus `partner-join-en`, `partner-code-en`, `partner-hub-documents-en`, `partner-contract-en`, `partner-redeem-en`, `partner-quality-ar`; ops: `ops-live`, `ops-requests`, `ops-partner-documents`, `ops-payout-run`. All captured from the live kits at 2×. Regenerate them whenever the kit changes; the pitch deck references them directly.

**UI kits** (`ui_kits/`) — **`marketing/` is the public website**, a faithful recreation of the live Next.js site the client supplied (`uploads/Bugsha-Website-Repo-v41/`): four pages (home, how it works, for businesses, our impact) behind a hash router, with `site.css` copied verbatim from the repo's `app/globals.css` and all photography copied into `assets/site/`. Its scroll scenes — cookie reveal, bag unpack, pinned app story, shared-table scale — are ported line for line from `app/page.tsx`. Two things are deliberately corrected against the repo, which shipped placeholder identity: the brand mark is the Kerchief rather than the repo's CSS-drawn rotated square, and the typeface is Archivo rather than Inter. Both should be fed back into the repo. The site is also written **pre-launch for two markets, Kuwait and Egypt** — Bugsha has not opened anywhere yet. Waitlist and partner-enquiry conversions, no cumulative statistics, no invented targets/dates/commercial terms, currency-free pricing, per-Bugsha arithmetic instead of invented history; see its README for what to swap back at launch. English only, because the repo has no Arabic route. See its README for the file-by-file source map. **`bugsha/` is the real one**: a fully click-through consumer app in the shipping identity, **at every stage** — language, intro, email-code sign-in, profile and city (including the city-waitlist state), dietary flags, then the marketplace, then every `order_status` (`held`, `reserved`, `redeemed`, `no_show`, `cancelled_partner`, `refunded`), receipt, report a problem, wallet, saved kitchens, profile, notification preferences and account deletion; market-aware for Kuwait (KNET, Apple Pay, card, KD) and Egypt (card, wallet, InstaPay, Fawry, cash, EGP with VAT). Every tab works: **Browse** (category chips, sorted list, closing-soon rail, how-it-works card), **Search** (live filtering by partner), **Bag detail** (quantity that recalculates price and caps at bags remaining, dietary chips, directions), **Payment** (KNET redirect state, Apple Pay), **Order + redemption** (code, slide-to-redeem, running-late banner that resolves, cancel-with-refund dialog), **Post-pickup** (impact stats, star review with free-text), **Map** (selectable price pins driving a linked card), **Orders** (active / past tabs with review history), **Me** (rescue log, language, appearance, notifications, payment methods, help). Plus filters and location sheets, an offline banner toggle, toasts, EN ⇄ العربية, light ⇄ dark, and Reset. Split across `data.jsx` (copy tables + fixtures), `screens.jsx` and `kit.jsx`. `cold-chain/` and `ticket/` are retained explorations that render browse / bag detail / redemption side by side. **`partner/` is the partner app** — the restaurant-facing surface in the shipping identity. It starts before the console: sign-in with an email code, then **a partner account opens only with a code issued by Bugsha** (enter the code, or request one), the application pre-filled from the code, and one status screen per `onboarding_status` (documents per market requirement, contract acceptance, branch setup with hours, first listing → active, rejected, suspended). The console is role-aware (`owner · manager · staff · accountant`) with fourteen views — the original seven plus Redeem (code or QR, late grace, undo), Schedules, Cash (Egypt), Quality (flags and customer reports), Documents (expiry pauses listings), Staff, and branch settings. The original seven: **Tonight** (live stats, attestation banner, orders board where Hand over flips a row to collected, pausable listing), **List a bundle** (the 60-second flow with customer preview), **My bundles** (saved presets per branch), **Payouts** (next transfer, bank on file, weekly gross/commission/net history), **Inspection log** (the compliance ledger with period + branch filters and CSV export), **Reviews** (rating summary + rows, framed as quality signal and inspection evidence), **Analytics** (sell-through, rescue volume, average price, best night, disposal cost avoided, per-branch table), **Branches** (branch states, staff, notification switches, language). Listings are real state: publish creates one, Edit opens a full editor (bilingual name, price, worth, quantity, window, nights, photo — library or upload), and pause/remove/hand-over all mutate the board. Split across `data.jsx`, `screens.jsx`, `onboarding.jsx`, `console.jsx`, `kit.jsx` — see its README. **`ops/` is the ops console** — the internal platform, English only, role-aware (`support_agent · ops_manager · finance · compliance · engineering · admin`): Live, partner requests and invite codes (the only way a partner account is opened), partners and partner detail (verify documents, approve, activate, suspend, contract and commission), orders (forced actions with reason codes), moderation, disputes and incidents with quality holds, payout runs with four-eyes approval, reconciliation, ledger and reports, users (restrict, credit, consented impersonation), market config with second-admin approval, feature flags, cities, notification templates, jobs, audit log. See its README.
